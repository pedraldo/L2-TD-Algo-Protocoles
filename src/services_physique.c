...
void de_canal(trame_t *trame) {
	...
	l_data = recvfrom(physique_socket, (char *)trame, sizeof(trame_t), 0, NULL, NULL);
	if (l_data < 0)
	{
		#ifndef UNIX
...
void vers_canal(trame_t *trame) {
	...
	if (trame->type == CON_REQ && perte_connexion) {
		printf("%s[PHY] perte de trame%s\n", RED, NRM);
		perte_connexion = 0; /* on ne perd qu'une fois */
		return;
	}
	
	/* perte reponse connexion ? */
	if (trame->type == CON_ACCEPT && perte_connexion) {
		printf("%s[PHY] perte de trame%s\n", RED, NRM);
		perte_connexion = 0; /* on ne perd qu'une fois */
		return;
	}
	...
	/* perte deconnexion ? */
	if (trame->type == CON_CLOSE && perte_deconnexion) {
		printf("%s[PHY] perte de trame%s\n", RED, NRM);
	...
	if (trame->type == CON_CLOSE_ACK && perte_deconnexion) {
		printf("%s[PHY] perte de trame%s\n", RED, NRM);
		perte_deconnexion = 0; /* on ne perd qu'une fois */
		return;
	}
	...
	new_trame = malloc(sizeof(trame_t));
	memcpy(new_trame, trame, sizeof(trame_t));
	
	
	/* erreur ? */
	if ( rand()/(float)RAND_MAX < physique_proba_erreur ) {
		printf("%s[PHY] generation d'une erreur dans la trame%s\n", RED, NRM);
		/* Complement à 1 du 1er octet du champ info : */
		new_trame->info[0] = ~( new_trame->info[0] );
	}
	...
	l_data = sendto(
		physique_socket,
		(char *) new_trame, sizeof(trame_t), 0,
